...
| SYS-03 | Replay duplica finalização, publicação, Coin/reward ou efeito assíncrono.  | `Idempotency-Key`, unique de origem, ledger/outbox append-only e consumer idempotente; teste repete e reordena a mesma intenção.              | Finalização usa `unique(user_id, operation_id)`, reaproveita o evento original e reverte estado se o enqueue falhar; handler real, reordenação e DLQ ainda faltam. | **Alto — Backend + Data.** O comando interno é aceito em staging sintético; nenhuma rota ou efeito econômico abre antes do handler idempotente e das provas restantes do FND-022. |
