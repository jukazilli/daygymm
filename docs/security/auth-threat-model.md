...
| ------- | ------------------------------------------------------------------------------------------------------------------------ | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| AUTH-01 | Enumeração por texto, status, tempo ou erro bruto no login/cadastro/recuperação.                                         | Copy genérica; contrato único de erro; resposta visual equivalente; detalhe do provedor não chega à UI/telemetria.                                                                                                          | Testes de componente e integração com conta existente/inexistente com mesma mensagem e forma de resposta.                           | Parcial: a conta íntegra foi comprovada, e COR-004 reproduziu uma recusa temporal `PGRST303` após login `200`; a recuperação seletiva passou localmente e aguarda staging.             | **Médio — Security + Produto.** Bloqueia conta externa até os testes hospedados passarem; aceita-se variação de latência do provedor somente em staging sintético. |
| AUTH-02 | Credential stuffing, criação automatizada e inundação de e-mail de confirmação/reset.                                    | Rate limits do Supabase; frequência mínima de 60 s por destinatário; cooldown de 80 s e `auth.resend` explícito; resposta 429 recuperável; Cloudflare Turnstile antes do beta externo; alerta por volume sem e-mail em log. | Integração prova limite/retry; E2E prova desafio quando acionado; configuração hospedada registrada sem segredo.                    | Parcial: Resend SMTP e limite hospedado foram ativados; o prazo recompõe suspensão com o processo vivo, mas COR-005 registra a perda após encerramento; CAPTCHA e alerta ainda faltam. | **Alto — Platform + Security.** Somente staging sintético sem CAPTCHA; beta externo bloqueado.                                                                     |
