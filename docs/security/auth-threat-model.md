...
Status do recorte: preparado para orientar o staging sintético em 13 de agosto
de 2026. FND-026 permanece `In Progress` até os controles de autenticação terem
prova executável e o modelo incorporar as demais fronteiras após
FND-011/FND-012.
...
| AUTH-04 | Roubo/replay de sessão por XSS, storage inseguro, dispositivo perdido ou refresh token copiado.                          | TLS; JWT de 1 h; refresh rotation/reuse de 10 s; CSP sem terceiros em auth; redaction; SecureStore mobile; logout local/global explícitos.                                      | Testes verificam storage por plataforma, ausência de token em URL/log, renovação, expiração, logout local e global.                 | Parcial: rotação/expiração estão declaradas; clientes e CSP ainda não existem.            | **Alto — Web/Mobile + Security.** Dados reais bloqueados até implementação; o access token pode sobreviver ao logout até expirar.                       |
...
## Bloqueios antes da implementação funcional
...
2. Produto/Privacy precisam aprovar identificadores e conteúdo das versões de
   Termos e Privacidade; hoje o banco aceita uma versão não vazia, mas nenhuma
   versão canônica foi definida.
3. Platform precisa cadastrar callbacks exatos de staging/web/mobile e provar
   a configuração hospedada.
4. FND-013 precisa implementar os controles e os dez testes acima. O documento
   sozinho não é evidência de funcionamento.
