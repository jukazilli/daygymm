...
| Google Cloud       | staging                        | Sessão operacional ativa; projeto host com dois principals em `roles/owner`; deploy usa Workload Identity Federation e service accounts dedicadas, sem chave persistente no GitHub. | MFA e recuperação das contas humanas não são expostos pela CLI; confirmação continua pendente.     | Billing ativo; budget `daygym-staging-monthly-budget` de R$ 50/mês confirmado com alertas em 50%, 80% e 100%.  | Billing, budget, IAM por contagem e serviços `daygym-api-staging`/`daygym-worker-staging` consultados em 13/08/2026.            | Fundação de staging pode continuar; produção e revisão nominal de menor privilégio permanecem bloqueadas. |
| Cloudflare         | staging                        | O projeto `daygym-web-staging` publica a branch `staging`, mas a sessão operacional local não está autenticada no Wrangler.                                                         | Owner, MFA, recovery e substituto não foram comprovados neste corte.                               | Pages permanece na capacidade atual; limites e owner de cobrança precisam do gate comercial.                   | Check Cloudflare Pages aprovado no SHA `011be87`; `wrangler 4.33.2 whoami` retornou sessão ausente em 13/08/2026.               | Deploy pelo GitHub continua; alteração de variáveis e recuperação exigem owner autenticado.               |
| Supabase           | staging                        | Migrations hospedadas comprovam acesso técnico pelo secret do GitHub; a sessão de Management API não consegue listar organização/projeto nem consultar chaves.                      | Owner, MFA, recovery e substituto não foram comprovados pela conta operacional atual.              | Projeto de staging existe na organização escolhida; plano e gatilho de upgrade permanecem no gate de produção. | Staging CI `31676393609` aplicou e testou migrations; `supabase projects/orgs list` falhou sem expor credenciais em 13/08/2026. | Banco sintético pode continuar; chave publicável, callbacks e administração permanecem bloqueados.        |
