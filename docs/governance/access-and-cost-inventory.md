...
| Supabase           | staging                        | Owner autenticado no painel; migrations seguem pelo secret do GitHub, configuração pública permanece no Cloudflare e SMTP Resend fica cifrado no store do Auth. O `Site URL` e dois redirects web exatos apontam para o Pages.                                                  | O desafio adicional do owner foi exercitado; recovery e substituto ainda não foram comprovados.    | Projeto de staging está no plano Free; SMTP customizado elevou o limite hospedado para 30 e-mails/h, com intervalo mínimo de 60 s por destinatário.                              | Cadastro e autenticação por senha retornaram HTTP 200; recuperação foi entregue e redirect validado em 13/08/2026.            | E-mail web está operacional; Data API ainda precisa expor minimamente `api` para concluir a sessão.       |
